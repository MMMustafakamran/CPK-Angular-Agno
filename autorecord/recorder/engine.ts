...
export class RecordingEngine {
  ...
  async recordPage(
    ...
    if (!health.frontendOk || !health.runtimeOk || !health.backendOk) {
      console.warn(`\n🔍 [Pre-flight Service Diagnostics]:`);
      if (!health.backendOk) {
        console.warn(
          `   🔴 Agno AgentOS Backend (port 8000) is unreachable: ${health.backendError}`,
        );
        console.warn(
      ...
      if (!health.runtimeOk) {
        console.warn(
          `   🔴 Copilot Runtime Node.js (port 8200) is unreachable: ${health.runtimeError}`,
        );
        console.warn(
          `      👉 Make sure to run: cd frontend && npm run runtime\n`,
        );
      }
      if (!health.frontendOk) {
        console.warn(
          `   🔴 Angular Frontend (port 4200) is unreachable: ${health.frontendError}`,
        );
        console.warn(
          `      👉 Make sure to run: cd frontend && npm start (or npm run dev)\n`,
        );
      }
    } else {
      console.log(`   ✅ Pre-flight checks passed: Angular (:4200), Runtime (:8200), Agno (:8000) are reachable.`);
    }
    ...
    // Attach informational console & request listeners
    ...
    try {
      if (config.docOnly) {
        ...
        try {
          ...
          // Wait for Angular app frame & chat element readiness
          console.log(`   ⏳ Waiting for Angular zoneless rendering & chat components...`);
          await page.waitForSelector('body', { timeout: 10000 }).catch(() => {});
          await page
            .waitForSelector(
              'copilot-chat, textarea, input[type="text"], input, [contenteditable="true"], app-demo-frame, [role="tablist"]',
