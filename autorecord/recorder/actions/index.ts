...
export const runStandardAction: PageActionHandler = async (
  ...
  // Ensure Angular signal receives the input event
  await page.evaluate((text) => {
    const textarea = document.querySelector('textarea, input[type="text"]') as HTMLTextAreaElement;
    if (textarea) {
      textarea.value = text;
  ...
  const sendBtn = page
    .locator(
      'copilot-chat-send-button button, copilot-chat-send-button, button[aria-label*="Send" i], button.copilot-chat-send-button, button[type="submit"], button:has-text("Send")',
    )
    .first();
