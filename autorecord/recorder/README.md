# Screen Recording Automation Suite — Angular 22 & CopilotKit + Agno

Automated Playwright test and recording engine for **Angular 22 (Zoneless + Signal-based)** and **CopilotKit + Agno AgentOS (Python)**.
...
└── recorder/
    ...
    ├── diagnostics.ts         # Pre-flight service checks (Angular :4200, Runtime :8200, Backend :8000)
    ├── ide/
    ...
    └── actions/
        ...
        ├── threads.action.ts       # Symmetrical demo: Headless list & drawer + Notepad license note
...
3. **Step 3 — Live Interactive Demonstration**:
   ...
   - Types tailored prompts with natural keystroke timing and executes interactions (e.g. Chat UI tab switching, Generative WeatherCard rendering, HITL approval gate, Headless custom transcript, Shared State priority updates, interactive file attachments, and Notepad developer explanations for A2UI, Threads, and Voice).

4. **Video Export**:
   - Clean runs saved to `autorecord/videos/AGNO-angular - <NN><FeatureName>.webm` (`✅ [PASS]`).
...
- **Terminal 1 — Agno AgentOS Backend (port 8000):**
  ```bash
  cd backend
  uv run main.py
  ```

- **Terminal 2 — Copilot Runtime (port 8200) & Angular Frontend (port 4200):**
  ```bash
  cd frontend
  npm run dev
  # Alternatively:
  # npm run runtime  (Terminal 2 - Port 8200)
