...
In this Angular + Agno AgentOS harness:
1. **Frontend (Angular 22)**: Port `4200` (`http://localhost:4200`)
2. **Copilot Runtime (Node.js)**: Port `8200` (`http://localhost:8200/api/copilotkit`)
3. **Backend (Python / Agno AgentOS)**: Port `8000` (`http://localhost:8000/agui`)
...
| `filename`          | Exported video filename: `AGNO-angular - <NN><FeatureName>`        |
| `docUrl`            | Official documentation URL for Step 1 (`/angular/agno/...`)        |
...
1. **`a2ui.action.ts`**:
   - Focuses input, types comparison prompt, and submits.
   - Waits for agent streaming text completion.
   - Slides up Windows 11 **Notepad** and types developer evaluation note explaining the missing frontend catalog requirement.
...
2. **`threads.action.ts`**:
   ...
   - Tests agent chat conversation beside drawer.
   - Slides up Windows 11 **Notepad** and types developer evaluation note on enterprise intelligence licensing.
   - Closes Notepad and completes recording.

3. **`attachments.action.ts`**:
...
4. **`voice.action.ts`**:
   ...
   - Slides up Windows 11 **Notepad** and types the audio transcription notice.
   - Closes Notepad and completes recording.
...
Videos are exported to `autorecord/videos/` as **1080p, 60fps WebM** files (`AGNO-angular - <NN><FeatureName>.webm`).
