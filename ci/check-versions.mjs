 ...
 *      (ci/automate.mjs runs `npm install` and `uv sync --upgrade`), so the
 *      versions under test move on their own. The snapshot written here is
 *      committed, which makes `git log -p ci/resolved-versions.json` the
 *      timeline: when a recording breaks, one diff says what moved overnight.
 *
 ...
 *
 * This script never edits package.json, pyproject.toml or a lockfile.
 */
 ...
 * `@ag-ui/agno` earns its place beside the CopilotKit pair because it is the
 * bridge this repo is built on — it is the only direct @ag-ui dependency the
 * frontend declares, and everything else @ag-ui arrives underneath
 * @copilotkit/runtime.
 */
const UPSTREAM_PROBES = ['@copilotkit/angular', '@copilotkit/runtime', '@ag-ui/agno'];
...
function collectResolved() {
  ...
  // directly rather than pulling in a parser for two fields. Note this reads
  // the COMMITTED lock: the workflow re-resolves the npm side but does not run
  // `uv sync --upgrade`, so the backend scope moves only when someone relocks.
