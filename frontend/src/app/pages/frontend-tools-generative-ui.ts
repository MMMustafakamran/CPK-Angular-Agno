    ...
    <div class="space-y-6">
      <ui-try-it>
        <p class="mt-1 text-[var(--ink-soft)]">
          The guide documents <strong>three</strong> generative UI paths and
          all three are mounted here. Open the demo and send
          <em>What's the weather in Tokyo?</em>, then
          <em>Change the background to violet</em>, then
          <em>Show me incident INC-4711, severity sev1.</em> — one prompt per
          path. The third is the guide's new first section, and it is where the
          findings are.
        </p>
      ...
      <ui-panel heading="Let the agent display one of your components">
        <p class="mb-3 text-sm text-[var(--ink-soft)]">
          ...
          process in <code>the Agno process</code> is untouched. Both snippets are
...
Show
      ...
      <ui-panel heading="registerRenderToolCall — server-side tool, browser renders">
        <p class="text-sm text-[var(--ink-soft)]">
          The Agno agent owns the <code>getWeather</code> tool; the browser only
      ...
      <ui-callout tone="warn" title="The renderer name must equal the tool name">
        ...
        agent's tool by exact string. The Agno agent in
        <code>backend/main.py</code> declares its tool as
